...
const SeriesMatches = ({ competitionId }) => {
    ...
    const fetchMatches = async () => {
      try {
        const response = await fetch(
          `/api/views/fixtures?CompetitionId=${competitionId}&UpcomingFixturesCount=30&InProgressFixturesCount=30&CompletedFixturesCount=30`
        );
